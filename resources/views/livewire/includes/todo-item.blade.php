        ...
        <div class="flex flex-row items-center">
            @if ($todo->completed)
                <input wire:click="toggle({{ $todo->id }})" type="checkbox"
                    class="m-2 text-teal-600 border-gray-300 rounded shadow-sm focus:border-teal-300 focus:ring focus:ring-teal-200 focus:ring-opacity-50"
                    wire:model="todo.completed" checked>
            @else
                <input wire:click="toggle({{ $todo->id }})" type="checkbox"
                    class="m-2 text-teal-600 border-gray-300 rounded shadow-sm focus:border-teal-300 focus:ring focus:ring-teal-200 focus:ring-opacity-50"
